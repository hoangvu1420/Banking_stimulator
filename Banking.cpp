...
struct bdsd
{
    ...
    int type;   // Loại giao dịch(1-Gửi tiền ; 2-Rút tiền)
...
struct cus
{
    ...
    string cmnd; // số cmnd
    int cond;   // tình trạng tài khoản (1 - bình thường ; 0 - bị khoá)        
...
date today() // Lấy ngày hiện tại sử dụng thư viện time.h
{
    date today;
    time_t t = time(NULL);
...
string convert_number(float number) {
...
{
    // Tạo một số tài khoản gồm 9 kí tự số bắt đầu bằng "704" và các chữ số còn lại được tạo ngẫu nhiên
    int sw1 = 0;
    string mkx;
    cus tm;
    do
...
void info(node *cur)
{
    ...
    if (cur->data.cond == 1)
        ...
        cout << "Normal" << endl;
    cout << "Balance: " << convert_number(cur->data.bal) << endl;
    cout << "Blance fluctuation records: " << endl;
    cout << "------------------" << endl;
    for (int i = 0; i < cur->data.bdsd.size(); i++)
    {
        cout << "Date: " << cur->data.bdsd[i].date.day << "/" << cur->data.bdsd[i].date.month << "/" << cur->data.bdsd[i].date.year << endl;
        cout << "Amount: ";
...
void transfer(node *cur)
{
    ...
    do
    {
        ...
        if (bd.type == 1)
            ...
            cout << "Withdraw";
        cout << endl;
        cout << "Amount: " << convert_number(bd.amou) << endl;
        bd.date = today();

        int cf;
        cout << "1 - Accept ; 0 - Decline >>> ";
        cin >> cf;
        if (cf == 0)
        {
        ...
        {
            if (bd.type == 2 && bd.amou > cur->data.bal)
            {
                cout << "Insufficient balance! Trasaction is cancelled" << endl;
                return;
            }
        ...
        cout << "----------------------------" << endl;
        cout << "Continue? (1 - Accept ; 0 - Decline) >>> ";
        cin >> sw;
    } while (sw);
    return;
}
